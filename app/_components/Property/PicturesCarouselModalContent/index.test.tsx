...
describe("PicturesCarouselModalContent", () => {
  ...
  it("affiche toutes les miniatures", () => {
    ...
    mockProperty.pictures.forEach((_, index) => {
      expect(
        screen.getByAltText(`Image secondaire ${index + 1} de Test Property`),
      ).toBeDefined();
    });
  });
  ...
  it("la miniature de l'image active a la bordure rouge", () => {
    ...
    const activeThumbnail = screen.getByAltText(
      "Image secondaire 1 de Test Property",
    );
  ...
  it("clique sur une miniature : met à jour la miniature active après la transition", async () => {
    ...
    const thumbnail3 = screen.getByAltText(
      "Image secondaire 3 de Test Property",
    );
    ...
    const updatedThumbnail = screen.getByAltText(
      "Image secondaire 3 de Test Property",
    );
  ...
  it("clique sur miniature non adjacente : la bonne image devient active", async () => {
    ...
    // Saut de img1 → img4
    const thumbnail4 = screen.getByAltText(
      "Image secondaire 4 de Test Property",
    );
  ...
  it("pas de bouton précédent/suivant si une seule image ", async () => {
    render(
      <PicturesCarouselModalContent
        property={mockPropertyOnePicture}
        onSubmitSuccess={onSubmitSuccess}
      />,
    );
  ...
  it("bouton suivant : passe à l'image 2", async () => {
    ...
    const thumbnail2 = screen.getByAltText(
      "Image secondaire 2 de Test Property",
    );
  ...
  it("bouton précédent : boucle vers la dernière image depuis la première", async () => {
    ...
    const lastThumbnail = screen.getByAltText(
      `Image secondaire ${mockProperty.pictures.length} de Test Property`,
    );
  ...
  it("bouton suivant depuis la dernière image : boucle vers la première", async () => {
    ...
    const firstThumbnail = screen.getByAltText(
      "Image secondaire 1 de Test Property",
    );
  ...
  it("touche ArrowLeft : passe à l'image précédente", async () => {
    ...
    fireEvent.click(
      screen
        .getByAltText("Image secondaire 2 de Test Property")
        .closest("button")!,
    );
    await act(async () => {
      vi.advanceTimersByTime(350);
